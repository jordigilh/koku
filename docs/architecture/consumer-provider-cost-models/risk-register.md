...
|----|------|----------|--------|
| R1 | Daily summary table migration on high-volume partitioned table | **Critical** | **Resolved by codebase pattern** — nullable `AddField` |
...
**Resolution**: Follow the established pattern. Add `context` as a
**nullable** field (`CharField`, `null=True`) to the daily summary
and all UI summary tables. `NULL` means "default context" (backward-
compatible — all existing rows are default). No batched backfill
needed. No `DEFAULT` clause needed.

This also simplifies the data model: instead of a NOT NULL column
requiring a migration-time default, NULL = default context is a
clean sentinel that requires no backfill.
...
**Resolution**: Ship context APIs universally, matching the existing
cost model pattern. Gating behind `ONPREM` would break the established
convention. Use Unleash feature flags for rollout control if needed
(same pattern as `IngressAccessPermission` grace period).
