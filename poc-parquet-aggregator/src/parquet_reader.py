...
class ParquetReader:
    ...
    def read_pod_usage_line_items(
        ...
        """Read OCP pod usage line items (daily).
        ...
        if not files:
            self.logger.warning(
                "No pod usage Parquet files found",
                prefix=s3_prefix
            )
        ...
        if streaming:
            return self.read_parquet_streaming(first_file, chunk_size)
        else:
    ...
    def read_node_labels_line_items(
        ...
        return self.read_parquet_file(files[0])
