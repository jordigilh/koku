...
### ❌ Missing / Bugs
1. **Node capacity**: TWO-LEVEL aggregation (max per interval, then sum per day)
2. **Empty node filter**: `AND node != ''`
3. **COALESCE logic**: For effective usage when NULL
4. **Month padding**: '1' vs '01' in filters
5. **Day filtering**: Specific days vs full month
...
## PRIORITY FIXES FOR POC

### P0 (Critical - Must Fix)
1. **Node filter**: Add `AND node != ''` (line 309)
2. **Month padding**: Use '01' not '1'
3. **Node capacity**: Aggregate from hourly intervals, not assume 24h

### P1 (High - Should Fix)
4. **COALESCE empty labels**: Handle NULL → `{}`
5. **Cluster capacity**: Verify aggregation
6. **JSON format**: Verify output format matches PostgreSQL
