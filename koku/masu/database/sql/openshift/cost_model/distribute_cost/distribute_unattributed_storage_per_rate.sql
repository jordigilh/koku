-- distribute_unattributed_storage_per_rate.sql (Phase 4 — IQ-9 Option 1)
--
-- Per-rate storage distribution: reads per-rate costs from rates_to_usage
-- for the 'Storage unattributed' namespace, distributes them proportionally
-- (by CPU or memory effective usage) to recipient namespaces,
-- and writes per-rate distributed rows back to rates_to_usage.
--
-- Mirrors distribute_unattributed_storage_cost.sql but operates at per-rate
