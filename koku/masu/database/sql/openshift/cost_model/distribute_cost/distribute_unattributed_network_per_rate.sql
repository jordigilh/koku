-- distribute_unattributed_network_per_rate.sql (Phase 4 — IQ-9 Option 1)
--
-- Per-rate network distribution: reads per-rate costs from rates_to_usage
-- for the 'Network unattributed' namespace, distributes them proportionally
-- (by CPU or memory effective usage) to recipient namespaces,
-- and writes per-rate distributed rows back to rates_to_usage.
--
-- Mirrors distribute_unattributed_network_cost.sql but operates at per-rate
-- granularity.  Recipients include both Pod and Storage data sources.
--
