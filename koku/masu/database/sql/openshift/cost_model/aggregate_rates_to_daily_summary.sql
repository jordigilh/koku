...
-- JSONB columns are retrieved via MIN() which is functionally correct
-- because all rows sharing a label_hash have identical JSONB values.
--
-- Parameters:
...
SELECT
    ...
    (array_agg(rtu.pod_labels))[1] AS pod_labels,
    (array_agg(rtu.volume_labels))[1] AS volume_labels,
    (array_agg(rtu.all_labels))[1] AS all_labels,
