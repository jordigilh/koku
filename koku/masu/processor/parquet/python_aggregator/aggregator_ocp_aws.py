"""
OCP-on-AWS Aggregator - Koku native implementation.
...
class OCPAWSAggregator:
    ...
    def __init__(
        ...
        """
        Initialize OCP-AWS aggregator with koku-native parameters.
        ...
        Args:
            ...
            cost_entry_bill_id: AWS cost entry bill ID (default 1)
            markup_percent: AWS markup percentage (default 0.0)
        """
        self.schema = schema_name
        self.provider_uuid = ocp_provider_uuid
        self.cluster_id = cluster_id
        self.cluster_alias = cluster_alias
        self.report_period_id = report_period_id
        self.aws_provider_uuid = aws_provider_uuid
        self.enabled_tag_keys = enabled_tag_keys
        self.cost_entry_bill_id = cost_entry_bill_id
        self.markup_percent = markup_percent
        self.logger = get_logger("aggregator_ocp_aws")

        # Performance configuration from environment
        self.use_streaming = os.getenv("POC_USE_STREAMING", "false").lower() == "true"
        self.chunk_size = int(os.getenv("POC_CHUNK_SIZE", "100000"))

        # Initialize components with koku-native API
        self.parquet_reader = ParquetReader(schema_name)
        self.aws_loader = AWSDataLoader(schema_name, logger=self.logger)
        self.resource_matcher = ResourceMatcher(logger=self.logger)
        self.tag_matcher = TagMatcher(logger=self.logger)
        self.disk_calculator = DiskCapacityCalculator(logger=self.logger)
        self.cost_attributor = CostAttributor(markup=markup_percent, logger=self.logger)
        self.network_handler = NetworkCostHandler(markup_percent=markup_percent, logger=self.logger)
        ...
        if self.use_streaming:
            self.streaming_processor = StreamingProcessor(
                logger=self.logger, name="ocp_aws_streaming", parallel_enabled=False, chunk_size=self.chunk_size
            )
        else:
