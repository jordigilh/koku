"""Storage aggregator for OCP storage usage data - Koku native implementation."""
...
class StorageAggregator:
    ...
    def __init__(
        ...
        """Initialize storage aggregator with koku-native parameters.
        ...
        self.use_arrow_compute = os.getenv("POC_USE_ARROW_COMPUTE", "false").lower() == "true"
    ...
    def aggregate(
        ...
        # Cleanup
        delete_intermediate = os.getenv("POC_DELETE_INTERMEDIATE_DFS", "true").lower() == "true"
        gc_after_agg = os.getenv("POC_GC_AFTER_AGGREGATION", "true").lower() == "true"
    ...
    def _format_output(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        result["source_uuid"] = self.provider_uuid
        result["report_period_id"] = self.report_period_id
        result["uuid"] = [str(uuid_lib.uuid4()) for _ in range(len(result))]  # Generate UUIDs
