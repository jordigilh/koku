"""
AWS Cost and Usage Report (CUR) Data Loader - Koku native implementation.

Reads AWS CUR Parquet files from S3/MinIO for OCP-on-AWS provider.
Uses koku's path building utilities and S3 access patterns.
...
class AWSDataLoader:
    ...
    def __init__(self, schema_name: str, logger=None):
        """
        Initialize AWS data loader with koku-native parameters.
        ...
        self.logger = logger or get_logger("aws_data_loader")
        ...
        self.parallel_workers = int(os.getenv("POC_PARALLEL_READERS", "4"))
    ...
    def read_aws_line_items_daily(
        ...
        with PerformanceTimer("Read AWS CUR line items", self.logger):
            # Build S3 path using koku's path format
            # Format: data/{schema}/AWS/source={provider_uuid}/year={year}/month={month}/
            s3_prefix = f"data/{self.schema}/AWS/source={provider_uuid}/year={year}/month={month}"
            ...
            if streaming:
                ...
                self.logger.info(f"Reading AWS CUR with {self.parallel_workers} parallel workers")
