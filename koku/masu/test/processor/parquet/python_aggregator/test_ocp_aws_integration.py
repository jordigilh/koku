...
class TestOCPAWSIntegration:
    ...
    def test_tag_matching_integration(
        ...
        # Create aggregator
        aggregator = OCPAWSAggregator(integration_config, ["openshift_cluster", "app"])
...
def test_network_cost_attribution_end_to_end(
    ...
    aws_data_with_network = pd.DataFrame(
        {
            ...
            "data_transfer_direction": [
                ...
                None,
                None,
            ],  # Network costs pre-classified
            "resource_id_matched": [
                True,
                True,
                True,
                True,
            ],  # All matched by resource ID
            "matched_resource_id": [
                "i-node1",
                "i-node2",
                "i-node1",
                "i-node2",
            ],  # Match OCP resource_id
            "tag_matched": [
                False,
                False,
                False,
                False,
            ],  # Matched by resource ID, not tags
        }
    )
    ...
    mock_matcher_instance.match_by_resource_id.return_value = (
        aws_data_with_network  # Already has resource_id_matched=True
    )
...
def test_timezone_mixing_in_network_costs(integration_config, sample_ocp_pod_usage, sample_ocp_storage_usage, mocker):
    ...
    tz_aware_timestamps = pd.to_datetime(
        [
    ...
    aws_data_with_tagged_ebs = pd.DataFrame(
        {
            ...
            "resourcetags": [
                ...
                json.dumps(
                    {
                        "openshift_cluster": "integration-test-cluster",
            ...
            "matched_ocp_namespace": [
                ...
                "",
                "backend",
            ],  # EBS tagged with openshift_project=backend
        }
    )
