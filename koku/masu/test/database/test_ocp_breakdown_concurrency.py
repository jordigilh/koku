...
class PopulateCostBreakdownConcurrencyTest(django.test.TransactionTestCase):
    ...
    def test_concurrent_breakdown_population_same_provider(self):
        ...
        print(f"\n[SPIKE][Finding A] thread results: {results}")
        ...
        print(f"[SPIKE][Finding A] expected (single-run) totals: {expected}")
        print(f"[SPIKE][Finding A] actual (concurrent) totals:   {actual}")
