...
Known residual gap (not fixed here, flagged for a follow-up decision):
...
exposed to this exact race. See ocp_report_db_accessor.py:1408.
"""
import threading
import uuid
...
class ProviderDeleteRTURaceTest(django.test.TransactionTestCase):
    ...
    def _run_protected_insert(self, results, barrier):
    ...
    def test_unprotected_rtu_insert_crashes_during_provider_delete(self):
        """Known residual gap: an uncoordinated writer still crashes even after the Option 2 fix.

        This documents populate_usage_rates_to_usage's exposure (it does not
        take _distribution_provider_lock today) rather than asserting it is
        fixed -- it is intentionally not fixed by this change. If this test
        ever starts failing (i.e. the insert stops crashing), it means that
        write path started participating in the lock and this test's
        expectations -- and its docstring -- need updating accordingly.
        """
        ...
        print(f"\n[SPIKE][Finding B, unprotected writer] results: {results}")
    ...
    def test_protected_rtu_insert_serializes_cleanly_against_provider_delete(self):
        ...
        print(f"\n[SPIKE][Finding B, protected writer] results: {results}")
